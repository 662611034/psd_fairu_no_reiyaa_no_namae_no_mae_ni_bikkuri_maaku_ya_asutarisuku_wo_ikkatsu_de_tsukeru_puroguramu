...
・「(目パチ口パクごと)」付きのボタンから書き出しを行うと、目パチや口パクのタブで行生成済みの
　スクリプトが挿入された状態で出力できます。トラックの値も処理されるため、出力した.anmは
　そのままご利用いただけます
'''
...
class Anm_Frame(ttk.Frame):
    ...
    def make_self(self):
        ...
        self.button_exports = []
        texts = ['既定の名前で書き出し', '名前を指定して書き出し', '.png抽出（仮）', '既定の名前で書き出し\n(目パチ口パクごと)', '名前を指定して書き出し\n(目パチ口パクごと)']
        for i in range(5):
