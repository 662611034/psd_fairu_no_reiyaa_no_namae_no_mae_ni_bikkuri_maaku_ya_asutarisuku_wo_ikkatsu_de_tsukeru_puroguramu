...
・.psdファイルの右クリックメニューからファイルを開くこともできます
　右クリックメニューに登録するにはregist.batを管理者権限で実行してください
　逆に解除する場合にはremove.batを管理者権限で実行してください
'''
...
・出力先は.psdファイルのあるフォルダに生成される「png_exported」フォルダ内の、
　指定したグループと同名のフォルダの中です
...
・出力された文字列は公式の場合と同じように利用者が.anmファイルに貼り付けて使うのが基本ですが、
　「目パチ口パクを含めて書き出す」にチェックを入れていれば.anmファイルの
　書き出し時に自動で挿入されます
